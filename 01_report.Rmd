---
title: "EMD proj 01 raport"
author: "Bartosz Nawrotek"
date: "1/2/2020"
...
```{r animate, fig.show='animate', cache=FALSE}
length <- data[sample(nrow(data), 1000), 1:2]
length <- data.frame(sapply(length, as.numeric))
p <- ggplot(data = length, aes(x = lp, y = length)) +
  geom_point() +
  geom_smooth()
p <- ggplotly(p)
p
...
```{r}
# Train
predictions_train <- predict(elastic_reg, train_ds)
eval_results(train_ds$length, predictions_train, train_ds) 

# Valid
predictions_valid <- predict(elastic_reg, valid_ds)
eval_results(valid_ds$length, predictions_valid, valid_ds) 
```
```{r}
library(mgcv)
generalized_lm <- glm(length ~ ., data = train_ds)

```

```{r}
predictions_train <- predict(generalized_lm, train_ds)
eval_results(train_ds$length, predictions_train, train_ds) 
```
